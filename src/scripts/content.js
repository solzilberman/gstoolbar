...
function exec_script(){
  browser.storage.local.get("gstoolbar_options").then((res) => {
    var sort_toggle_enabled = res.gstoolbar_options.sort_toggle;
    var show_toggle_enabled = res.gstoolbar_options.show_toggle;
    if (sort_toggle_enabled) {
      run_sort_toggle();
    } 
    if (show_toggle_enabled) {
      run_show_toggle();
    }
  });
}
...
browser.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.message === "load" || request.message === "update") {
      exec_script();
    }
    
    if ("refresh" in request & request.refresh === true) {
      refresh_page();
    }
});

function run_sort_toggle(){
